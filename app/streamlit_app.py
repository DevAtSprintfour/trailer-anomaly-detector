...
from cp_packer import CpPacker
...
with st.sidebar.expander("Floor dimensions by trailer category (tunable)", expanded=False):
    st.caption(
        ...
        "container size: dance-chamber length, general-chamber length, and the "
        "shared interior width. All categories start at the same default and "
    ...
    for cat in ALL_CATEGORIES:
        ...
        dl = ca.number_input(
            "Dance length (in)",
            value=float(defaults["dance_length"]),
            min_value=1.0,
            step=1.0,
            key=f"dl_{cat}",
        )
        gl = cb.number_input(
            "General length (in)",
            value=float(defaults["general_length"]),
            min_value=1.0,
            step=1.0,
            key=f"gl_{cat}",
        )
...
# The soft-line packing model has no length overflow (every trailer packs; see
# cp_packer), so there are no season-wide ⚠ overflow badges to precompute. These
# stay empty; anomalies (width-impossible items) surface per-trailer on reprocess.
overflow_pairs: set = set()  # (race_id, trailer_name)
overflow_trailers: set = set()
overflow_races: set = set()

eq_meta = (
    work[["equipment_id", "serial_number", "equipment_desc", "eq_length", "eq_width"]]
...
for f in build_used_floors(scoped, geom, category_geom, dim_overrides, padding=gap):
...
for tname in sorted(sel_trailers, key=_trailer_sort_key):
    ...
    with st.container(border=True):
        ...
        tfloor = tfloors[0]
        container = tfloor.container
        trailer_id = tfloor.trailer_id
        rid = int(sel_race)

        dance_items = tfloor.items_on(FLOOR_DANCE)
        general_items = tfloor.items_on(FLOOR_GENERAL)
        ...
        fig = RENDERER.figure(container, tfloor.items, verdict)
        ...
        for eid in sorted(ids_here):
            ...
            if not needs_action:
                continue
            floor_key = (
                FLOOR_DANCE if any(it.equipment_id == eid for it in dance_items) else FLOOR_GENERAL
            )
            meta = eq_meta.loc[eid] if eid in eq_meta.index else None
            orig_L = (
                float(meta["eq_length"])
                if meta is not None and pd.notna(meta["eq_length"])
                else 0.0
            )
            orig_W = (
                float(meta["eq_width"]) if meta is not None and pd.notna(meta["eq_width"]) else 0.0
            )
            cur_L, cur_W = dim_overrides.get(eid, (orig_L, orig_W))
            desc = meta["equipment_desc"] if meta is not None else eid
            st.markdown(
                f"**#{eid}** — {desc} · {status_badge(v['status'])} · {floor_badge(floor_key)}",
...
fails = equipment_table(
    {e for e in scoped_eids if verdict.get(e, {}).get("status") == FAIL}, verdict
)
if not fails.empty:
    st.write(f"**{len(fails)}** equipment with stored sizes that contradict a working floor load.")
    st.dataframe(fails.style.map(color_status, subset=["status"]), width="stretch", hide_index=True)
    st.download_button(
        "Download good-fail list (CSV)",
        fails.to_csv(index=False),
        "good_fail_list_2026.csv",
        "text/csv",
    )
else:
...
if amb_ids:
    with st.expander(f"Ambiguous ({len(amb_ids)}) — scan the group"):
        ...
        st.dataframe(
            amb.style.map(color_status, subset=["status"]), width="stretch", hide_index=True
        )
...
if unk_ids:
    with st.expander(f"Unknown dims ({len(unk_ids)})"):
        ...
        st.dataframe(
            unk[["equipment_id", "serial", "description", "reason"]],
            width="stretch",
            hide_index=True,
        )
